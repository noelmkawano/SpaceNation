...
public class SpaceshipRV : MonoBehaviour {
    ...
    private Vector3 initialOrientationn; 
	...
	void Start () {
        ...
        initialOrientationn = transform.rotation * axisN;
	}
	...
	public void Rotate(float dTheta, Vector3 axis) {
        ...
        if (rocketEngine != null) {
            ...
            rocketEngine.SetRotation(Quaternion.FromToRotation(initialOrientationn, orientation));
        }
        if (thrustSize != 0) {
	...
	/// integrator timeslice. The SpaceshipRV maintains a 
	/// </summary>
